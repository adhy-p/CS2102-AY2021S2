...
CREATE OR REPLACE FUNCTION find_instructors(
    id_course INTEGER, date_session DATE, session_start_hour TIME
    ) RETURNS TABLE(eid INTEGER, name VARCHAR(50)) AS $$
DECLARE
    course_duration INTEGER;
BEGIN
    IF (id_course NOT IN (SELECT C.course_id FROM Courses C)) THEN
...
/* Routine no. 11 - 15, @author: Jusuf Nathanael */
...
FROM (((((Redeems NATURAL JOIN Credit_Cards) NATURAL JOIN Sessions) NATURAL JOIN (SELECT fees, course_id, launch_date FROM Offerings) O) NATURAL JOIN Courses) NATURAL JOIN Instructors) NATURAL JOIN (SELECT eid, name AS insName FROM Employees) Ins
WHERE cust_id = cust_id_in AND (SELECT CURRENT_DATE) <= session_date
UNION
SELECT name, fees, session_date, start_time, duration, insName
FROM (((((Registers NATURAL JOIN Credit_Cards) NATURAL JOIN Sessions) NATURAL JOIN (SELECT fees, course_id, launch_date FROM Offerings) O) NATURAL JOIN Courses) NATURAL JOIN Instructors) NATURAL JOIN (SELECT eid, name AS insName FROM Employees) Ins
WHERE cust_id = cust_id_in AND (SELECT CURRENT_DATE) <= session_date
EXCEPT
SELECT name, fees, session_date, start_time, duration, insName
FROM (((((Cancels NATURAL JOIN Credit_Cards) NATURAL JOIN Sessions) NATURAL JOIN (SELECT fees, course_id, launch_date FROM Offerings) O) NATURAL JOIN Courses) NATURAL JOIN Instructors) NATURAL JOIN (SELECT eid, name AS insName FROM Employees) Ins
WHERE cust_id = cust_id_in AND (SELECT CURRENT_DATE) <= session_date
ORDER BY session_date, start_time
$$ LANGUAGE sql;
...
CREATE OR REPLACE FUNCTION offering_deadline() RETURNS TRIGGER AS $$
BEGIN
    IF (NEW.registration_deadline >= NEW.start_date + INTERVAL '10 days') THEN
        RETURN NEW;
    END IF;
    RETURN NULL;
END;
...
CREATE TRIGGER delete_session
BEFORE INSERT ON Registers
FOR EACH ROW EXECUTE FUNCTION offering_deadline();
